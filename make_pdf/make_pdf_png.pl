...
sub combine_images() {
  ...
  my $subdir = shift;
  my $pdf = shift;
  # File::Finder steps "plain files named *.png"
  my $all_jpegs = File::Finder->type('f')->left->name("*.png")->or->name("*.PNG")->right;
...
__END__
my $image = $pdf->image_jpeg( "$dir/$png" );
    $pdf->page( $image->width, $image->height );
    $pdf->image( $image, 0, 0 );
}
